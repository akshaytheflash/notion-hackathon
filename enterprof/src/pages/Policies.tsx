...
export default function Policies() {
  ...
  function getPolicyName(policy: Policy): string {
    ...
    if (nameProp?.title?.[0]?.text?.content) {
      return nameProp.title[0].text.content;
    }
  ...
  function getPolicyId(policy: Policy): string {
    ...
    if (idProp?.rich_text?.[0]?.text?.content) {
      return idProp.rich_text[0].text.content;
    }
    return policy.id;
  }

  function getDepartment(policy: Policy): string {
    return policy.properties?.Department?.select?.name || "N/A";
  }

  function getLimit(policy: Policy): number {
    return policy.properties?.Limit?.number || 0;
  }

  function getRequiredAction(policy: Policy): string {
    return policy.properties?.["Required Action"]?.select?.name || "N/A";
  }

  function isActive(policy: Policy): boolean {
    return policy.properties?.Active?.checkbox || false;
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--color-dim)" }}>
            Active policies and enforcement rules for autonomous spending limits.
          </p>
        </div>
        <button
          onClick={() => setShowAddForm(!showAddForm)}
          className="rounded-md px-5 py-2.5 text-sm font-semibold transition-all duration-200 hover:opacity-90"
          style={{
      ...
      {error && (
        <div
          className="rounded-lg border px-4 py-3 text-xs font-mono mb-4"
          style={{
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., DATA_ACCESS_LIMIT"
                className="rounded px-3.5 py-2.5 text-sm w-full"
                style={{
            ...
            <div>
              ...
              >
                <option>SPENDING_LIMIT</option>
                <option>TIME_LIMIT</option>
                <option>ACCESS_CONTROL</option>
            ...
            <div>
              ...
              <select
                value={newPolicy.required_action}
                onChange={(e) =>
                  setNewPolicy({
                    ...newPolicy,
              ...
              >
                <option>HUMAN_APPROVAL_REQUIRED</option>
                <option>MANAGER_APPROVAL_REQUIRED</option>
          ...
          <div className="mt-4 flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setNewPolicy({ ...newPolicy, active: e.target.checked })
                }
                className="rounded"
              />
            ...
            <button
              ...
              disabled={adding || !newPolicy.name.trim()}
              className="rounded-md px-5 py-2.5 text-sm font-semibold transition-all duration-200 hover:opacity-90 disabled:opacity-50"
              style={{
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="rounded-lg border p-4 skeleton h-24"
          ...
          >
            ...
            <p className="text-sm mt-1" style={{ color: "var(--color-dim)" }}>
              Add a policy to enforce spending limits.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <span
                      ...
                      style={{
                        color: isActive(policy)
                          ? "var(--color-signal-green)"
                          : "var(--color-dim)",
                        border: `1px solid ${isActive(policy) ? "var(--color-signal-green)" : "var(--color-dim)"}`,
                      }}
                    >
                      {isActive(policy) ? "ACTIVE" : "INACTIVE"}
                    </span>
                    <span
      ...
      <div>
        <h3
          ...
          style={{ color: "var(--color-text)" }}
        >
          Recent Policy Evaluations ({evaluations.length})
        </h3>
        {evaluations.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm mt-1" style={{ color: "var(--color-dim)" }}>
              Run the demo to trigger spending limit checks.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {evaluations.map((evaluation) => {
              ...
              return (
                ...
                >
                  ...
                  {typeof payload.violation === "string" &&
                    payload.violation && (
                      <div className="mt-3">
                        <label
                          className="text-xs font-mono uppercase tracking-wider mb-0.5 block"
                          style={{ color: "var(--color-dim)" }}
                        >
                          Violation
                        </label>
...
function PolicyMetric({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <label
        className="text-xs font-mono uppercase tracking-wider block mb-0.5"
        style={{ color: "var(--color-dim)" }}
      >
        {label}
      </label>
