...
export default function WorkflowDetailPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-2 text-xs font-mono" style={{ color: "var(--color-dim)" }}>
      ...
      >
        ...
        <div className="mt-6">
          <label className="text-[10px] font-mono uppercase tracking-wider mb-2 block" style={{ color: "var(--color-dim)" }}>Pipeline Progress</label>
          <div
      ...
      >
        <div className="px-4 py-3 border-b" style={{ borderColor: "var(--color-hairline)" }}>
          <h2 className="font-semibold text-sm tracking-wide">Event Timeline</h2>
          <p className="text-[11px] mt-0.5" style={{ color: "var(--color-dim)" }}>
        ...
        <div className="divide-y" style={{ borderColor: "var(--color-hairline)" }}>
          {events.length === 0 ? (
            <div className="px-4 py-8 text-center">
              <p className="text-xs" style={{ color: "var(--color-dim)" }}>No events recorded for this workflow.</p>
            </div>
          ) : (
            events.map((ev) => (
              <div
                key={ev.id}
                className="px-4 py-3 flex items-start gap-4 animate-slide-in"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <pre className="text-[11px] font-mono whitespace-pre-wrap break-all" style={{ color: "var(--color-muted)" }}>
                    {JSON.stringify(ev.payload_json, null, 2)}
                  </pre>
                </div>
                <span className="text-[10px] font-mono shrink-0" style={{ color: "var(--color-dim)" }}>
