  ...
  return (
    <div className="p-6">
      ...
      {!loading && !error && decisions.length === 0 && (
        ...
        >
          <p className="text-sm" style={{ color: "var(--color-dim)" }}>No decisions recorded yet.</p>
          <p className="text-xs mt-1" style={{ color: "var(--color-dim)" }}>Run the demo to generate AI agent decisions.</p>
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                {typeof payload.decision === "string" && (
                  <div className="mb-2">
                    <label className="text-[10px] font-mono uppercase tracking-wider mb-0.5 block" style={{ color: "var(--color-dim)" }}>Decision</label>
                    <p className="text-xs" style={{ color: "var(--color-text)" }}>{payload.decision}</p>
                  </div>
                )}

                {typeof payload.reasoning_summary === "string" && (
                  <div className="mb-2">
                    <label className="text-[10px] font-mono uppercase tracking-wider mb-0.5 block" style={{ color: "var(--color-dim)" }}>Reasoning</label>
                    <p className="text-xs leading-relaxed" style={{ color: "var(--color-muted)" }}>{payload.reasoning_summary}</p>
                  </div>
                )}

                {typeof payload.evidence === "string" && (
                  <div className="mb-2">
                    <label className="text-[10px] font-mono uppercase tracking-wider mb-0.5 block" style={{ color: "var(--color-dim)" }}>Evidence</label>
                    <p className="text-xs leading-relaxed" style={{ color: "var(--color-muted)" }}>{payload.evidence}</p>
                  </div>
                )}

                {typeof payload.confidence === "number" && (
                  <div className="flex items-center gap-2 mt-2">
                    <label className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "var(--color-dim)" }}>Confidence</label>
                    <div className="w-24 h-1.5 rounded-full" style={{ backgroundColor: "var(--color-hairline)" }}>
                      <div
